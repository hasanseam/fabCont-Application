...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang"  ]; then
	...
	CC_SRC_PATH=github.com/hyperledger/fabric-samples/chaincode/fabcont/go
	echo Vendoring Go dependencies ...
	pushd ../chaincode/fabcont/go
	GO111MODULE=on go mod vendor
	popd
	echo Finished vendoring Go dependencies
elif [ "$CC_SRC_LANGUAGE" = "java" ]; then
	CC_RUNTIME_LANGUAGE=java
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/fabcont/java/build/install/fabcont
  echo Compiling Java code ...
  pushd ../chaincode/fabcont/java
  ./gradlew installDist
  popd
  echo Finished compiling Java code
elif [ "$CC_SRC_LANGUAGE" = "javascript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/fabcont/javascript
elif [ "$CC_SRC_LANGUAGE" = "typescript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/fabcont/typescript
	echo Compiling TypeScript code into JavaScript ...
	pushd ../chaincode/fabcont/typescript
	npm install
	npm run build
	popd
...
REGEX='Package ID: (.*), Label: fabcontv1'
if [[ `${PEER0_ORG1} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  PACKAGE_ID_ORG1=${BASH_REMATCH[1]}
else
  echo Could not find package ID for fabcontv1 chaincode on peer0.org1.example.com
  exit 1
fi
...
${PEER0_ORG1} lifecycle chaincode approveformyorg \
  ...
  --channelID mychannel \
  --name fabcont \
  --collections-config /opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/fabcont/collections_config.json \
  --version 1.0 \
...
${PEER0_ORG2} lifecycle chaincode install fabcont.tar.gz

echo "Installing smart contract on peer1.org2.example.com"
${PEER1_ORG2} lifecycle chaincode install fabcont.tar.gz
...
if [[ `${PEER0_ORG2} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  ...
  echo Could not find package ID for fabcontv1 chaincode on peer0.org2.example.com
  exit 1
fi
...
Next, use the Fabcont applications to interact with the deployed Fabcont contract.
The Fabcont applications are available in multiple programming languages.
